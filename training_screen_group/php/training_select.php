...
try {
    // 1) 日付の取得：URLパラメータがあればそれを使い、なければ今日の日付にする
    $selected_date = isset($_GET['date']) ? $_GET['date'] : date("Y-m-d");
    $current_session_id = null;

    // 2) 選択された日付のセッションID取得/作成
    $stmt_session = $pdo->prepare("SELECT session_id FROM workout_sessions WHERE user_id = ? AND date = ?");
    $stmt_session->execute([$user_id, $selected_date]);
    ...
    if ($session_data) {
        ...
        $stmt_insert_session->execute([$user_id, $selected_date]);
    ...
    <div class="week-calendar">
        <?php foreach ($week_dates as $date): ?>
            <div class="day-item<?php echo $date['is_selected'] ? ' active' : ''; ?>" 
                 onclick="location.href='?date=<?= $date['full_date'] ?>'">
        ...
        <?php else: ?>
            <?php foreach ($trainings as $index => $training): ?>
                <?php
                    ...
                    if ($currSup !== null) {
                        if ($prevSup !== $currSup && $nextSup === $currSup) $cardClasses .= ' superset-top';
                        elseif ($prevSup === $currSup && $nextSup !== $currSup) $cardClasses .= ' superset-bottom';
                        elseif ($prevSup === $currSup && $nextSup === $currSup) $cardClasses .= ' superset-middle';
                    }
                ?>
                <div class="<?php echo $cardClasses; ?>" data-training-id="<?php echo (int)$training['training_id']; ?>" data-type-ids="<?php echo htmlspecialchars($training['type_ids'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <button class="add-training-btn" onclick="location.href='training_list.php?date=<?= $selected_date ?>'">トレーニング追加</button>
